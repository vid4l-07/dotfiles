...
param="$(printf ' %q' "$@")"
if [ -z "$@" ]; then
    cmd="$HOME/.config/polybar/naivecalendar/naivecalendar.py"
else
    cmd="$HOME/.config/polybar/naivecalendar/naivecalendar.py $(printf ' %q' "$@")"
fi
...
THEME_SOURCE_PATH="$HOME/.config/polybar/naivecalendar/themes"
...
# otherwise, look in cache
    ...
    THEME="$THEME_SOURCE_PATH/theme_base.rasi"
fi
